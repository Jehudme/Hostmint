...
function statsExtracterPlugin(): PluginOption {
  ...
  return {
    ...
    async writeBundle(options: OutputOptions, bundle: { [fileName: string]: AssetInfo | ChunkInfo }) {
      ...
      parseImports(path.resolve(themeOptions.frontendGeneratedFolder, 'flow', 'generated-flow-imports.js'), generatedImportsSet);
      parseImports(path.resolve(themeOptions.frontendGeneratedFolder, 'app-shell-imports.js'), generatedImportsSet);
      ...
      const isGeneratedWebComponentResource = (id: string) =>
        id.startsWith(themeOptions.frontendGeneratedFolder.replace(/\\/g, '/')) && id.match(/.*\/flow\/web-components\//);
