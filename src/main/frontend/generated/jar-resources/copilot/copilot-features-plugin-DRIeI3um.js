...
const n = a => s.userInfo?.copilotExperimentalFeatureFlag === !0 && l.isExperimentalFeatureEnabled(a),
  F = {
    id: 'theme-from-image',
...
var M = Object.defineProperty,
  ...
  d = (a, e, t, i) => {
    for (var r = i > 1 ? void 0 : i ? S(e, t) : e, c = a.length - 1, p; c >= 0; c--) (p = a[c]) && (r = (i ? p(e, t, r) : p(r)) || r);
...
let m = class extends C {
    ...
    return o`
      <div class="flex flex-col gap-6 px-4 py-0.5">
        ...
        <div class="flex flex-col gap-1">
          ${
            a
              ? o`<h3 class="font-semibold my-0 text-sm">Copilot Experimental Features</h3>
                <div class="border-dashed flex flex-col divide-y">
  ...
  toggleFeatureFlag(a, e) {
    ...
    (v('use-feature', { source: 'toggle', enabled: t, id: e.id }),
      g.frontendConnection
        ? (g.frontendConnection.send('setFeature', { featureId: e.id, enabled: t }),
          e.requiresServerRestart && s.toggleServerRequiringFeatureFlag(e),
          y({
            type: T.INFORMATION,
          ...
          e.id === 'copilotExperimentalFeatures' && s.userInfo && s.setUserInfo({ ...s.userInfo, copilotExperimentalFeatureFlag: t }))
        : g.log('error', `Unable to toggle feature ${e.title}: No server connection available`));
  }
...
d([x()], m.prototype, 'toggledFeaturesThatAreRequiresServerRestart', 2);
m = d([f('copilot-features-panel')], m);
let b = class extends I {
  ...
  render() {
    if (s.serverRestartRequiringToggledFeatureFlags.length === 0) return u;
    if (!q()) return u;
...
d([x()], b.prototype, 'serverRestarting', 2);
b = d([f('copilot-features-actions')], b);
const D = {
    header: 'Features',
...
export { b as CopilotFeaturesActions, m as CopilotFeaturesPanel };
