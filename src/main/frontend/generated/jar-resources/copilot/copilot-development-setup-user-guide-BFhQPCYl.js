...
var O = Object.defineProperty,
  ...
  g = (e, t, a, s) => {
    for (var o = s > 1 ? void 0 : s ? U(t, a) : t, i = e.length - 1, d; i >= 0; i--) (d = e[i]) && (o = (s ? d(t, a, o) : d(o)) || o);
    return (s && o && O(t, a, o), o);
  };
const S = 'https://github.com/JetBrains/JetBrainsRuntime/releases';
function _(e, t) {
  if (!t) return !0;
  const [a, s, o] = t.split('.').map(v => parseInt(v)),
    [i, d, h] = e.split('.').map(v => parseInt(v));
  if (a < i) return !0;
  if (a === i) {
    if (s < d) return !0;
    if (s === d) return o < h;
...
let u = class extends V {
  constructor() {
    (super(),
      (this.javaPluginSectionOpened = !1),
      (this.hotswapSectionOpened = !1),
      (this.hotswapTab = 'hotswapagent'),
      (this.downloadStatusMessages = []),
      (this.downloadProgress = 0),
      (this.onDownloadStatusUpdate = this.downloadStatusUpdate.bind(this)),
      (this.handleESC = e => {
      ...
      this.reaction(
        ...
        () => {
          (p.idePluginState &&
            (!p.idePluginState.ide || !p.idePluginState.active
              ? (this.javaPluginSectionOpened = !0)
              : (!/* @__PURE__ */ new Set(['vscode', 'intellij']).has(p.idePluginState.ide) || !p.idePluginState.active) &&
                (this.javaPluginSectionOpened = !1)),
            l.jdkInfo && b() !== 'success' && (this.hotswapSectionOpened = !0));
        },
        { fireImmediately: !0 },
      ));
  }
  ...
  renderPluginSection(e) {
    ...
    let a, s;
    e.vscode || e.intellij
      ? e.idePluginInstalled
        ? ((a = `Plugin for ${t} installed`), (s = this.renderPluginInstalledContent()))
        : ((a = `Plugin for ${t} not installed`), (s = this.renderPluginIsNotInstalledContent(e)))
      : e.eclipse
        ? ((a = e.idePluginInstalled ? 'Eclipse plugin installed' : 'Eclipse plugin not installed'),
          (s = e.idePluginInstalled ? this.renderPluginInstalledContent() : this.renderEclipsePluginContent()))
        : ((a = 'No IDE found'), (s = this.renderNoIdePluginContent()));
  ...
  renderPluginIsNotInstalledContent(e) {
    let t = null,
      a = 'Install from Marketplace';
    return (
      e.intellij
        ? ((t = M), (a = 'Install from JetBrains Marketplace'))
        : e.vscode
          ? ((t = R), (a = 'Install from VSCode Marketplace'))
          : e.eclipse && ((t = I), (a = 'Install from Eclipse Marketplace')),
      n`
      <div class="flex flex-col gap-2 items-start pb-4 px-4">
  ...
  renderHotswapSection(e) {
    ...
    return (
      a === 'success'
        ? ((o = r.checkCircle), (d = 'Java Hotswap is enabled'))
        : a === 'warning'
          ? ((o = r.warning), (d = 'Java Hotswap is not enabled'))
          : a === 'error' && ((o = r.warning), (d = 'Java Hotswap is partially enabled')),
      this.hotswapTab === 'jrebel'
        ? t.jrebel
          ? (i = this.renderJRebelInstalledContent())
          : (i = this.renderJRebelNotInstalledContent())
        : e.intellij
          ? (i = this.renderIntelliJHotswapHint())
          : e.vscode
            ? (i = this.renderVSCodeHotswapHint())
            : (i = this.renderHotswapAgentNotInstalledContent(e)),
      n` <vaadin-details
      theme="reverse"
      ...
      <div>
        ${
          s !== 'none'
            ? n`${s === 'jrebel' ? this.renderJRebelInstalledContent() : this.renderHotswapAgentInstalledContent()}`
            : n`
              <vaadin-tabs
  ...
  renderHotswapAgentJdkSection(e) {
    const t = l.jdkInfo?.extendedClassDefCapable,
      a = this.downloadStatusMessages?.[this.downloadStatusMessages.length - 1] === P;
    return n`
      <vaadin-details theme="reverse" .opened="${!t}">
        ...
        <div class="flex flex-col gap-2 pb-2 px-2 text-secondary">
          ...
          <ul class="m-0 ps-4">
            ...
            ${
              e.vscode
                ? n` <li>
                  ...
                  ${
                    this.downloadProgress > 0
                      ? n`<vaadin-progress-bar
                        .value="${this.downloadProgress}"
                        min="0"
                        max="1"></vaadin-progress-bar>`
                      : c
                  }
                  <ul>
                    ...
                    ${
                      a
                        ? n`<h3 class="font-semibold my-0 text-sm">
            ...
            <li>
              ${
                e.intellij || e.vscode
                  ? n`If there is a problem, you can manually
                    <a target="_blank" href="${S}">download JetBrains Runtime JDK</a> and set up your
                    debug configuration to use it.`
                  : n`<a target="_blank" href="${S}">Download JetBrains Runtime JDK</a> and set up
  ...
  renderInstallHotswapAgentJdkSection(e) {
    const t = l.jdkInfo?.hotswapAgentFound,
      a = l.jdkInfo?.extendedClassDefCapable;
    return n`
      <vaadin-details theme="reverse" .opened="${!t}">
  ...
  renderHotswapAgentVersionSection() {
    if (!l.jdkInfo?.hotswapAgentFound) return c;
    const e = l.jdkInfo?.hotswapVersionOk,
      t = l.jdkInfo?.hotswapVersion,
      a = l.jdkInfo?.hotswapAgentLocation;
    return n`
      <vaadin-details theme="reverse" .opened="${!e}">
  ...
  async downloadJetbrainsRuntime(e) {
    return (
      ...
      e.preventDefault(),
      (this.downloadStatusMessages = []),
      H(`${k}set-up-vs-code-hotswap`, {}, t => {
        t.data.error
          ? (D('Error downloading JetBrains runtime', t.data.error),
            (this.downloadStatusMessages = [...this.downloadStatusMessages, 'Download failed']))
          : (this.downloadStatusMessages = [...this.downloadStatusMessages, P]);
      })
    );
  }
...
g([m()], u.prototype, 'javaPluginSectionOpened', 2);
g([m()], u.prototype, 'hotswapSectionOpened', 2);
g([m()], u.prototype, 'hotswapTab', 2);
g([m()], u.prototype, 'downloadStatusMessages', 2);
g([m()], u.prototype, 'downloadProgress', 2);
u = g([j(u.NAME)], u);
...
C = g([j('copilot-development-setup-footer-actions')], C);
const w = {
...
f.addPanel(w);
export { C as CopilotDevelopmentSetupFooterActions, u as CopilotDevelopmentSetupUserGuide, w as copilotDevelopmentSetupPanelConfig };
