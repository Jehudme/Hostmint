...
var _ = Object.defineProperty,
  ...
  u = (e, t, a, i) => {
    for (var o = i > 1 ? void 0 : i ? B(t, a) : t, l = e.length - 1, r; l >= 0; l--) (r = e[l]) && (o = (i ? r(t, a, o) : r(o)) || o);
...
let d = class extends q {
  constructor() {
    (super(...arguments),
      ...
      (this.errorHandlersAdded = !1));
  }
  connectedCallback() {
    if (
      (super.connectedCallback(),
      this.onCommand('log', e => {
        this.handleLogEventData({ type: e.data.type, message: e.data.message });
      }),
      this.onEventBus('log', e => this.handleLogEvent(e)),
      this.onEventBus('update-log', e => this.updateLog(e.detail)),
      this.onEventBus('notification-shown', e => this.handleNotification(e)),
      this.onEventBus('clear-log', () => this.clear()),
      this.reaction(
  ...
  handleLogEventData(e) {
    this.log(e.type, e.message, !!e.internal, e.details, e.link, c(e.expandedMessage), c(e.expandedDetails), e.id);
  }
  activate() {
    ((this.unreadErrors = !1),
    ...
    return (
      ...
      <li
        ...
        <span class="message" @click=${() => this.toggleExpanded(e)}>
          ...
          ${
            e.expanded
              ? n` <span class="secondary"> ${e.expandedDetails ?? e.details} </span>`
              : n` <span class="secondary" ?hidden="${!e.details && !e.link}">
                ${c(e.details)}
  ...
  log(e, t, a, i, o, l, r, E) {
    ...
    for (this.messages.push(g); this.messages.length > d.MAX_LOG_ROWS; ) this.messages.shift();
    return (
      this.requestUpdate(),
      this.updateComplete.then(() => {
        const m = this.renderRoot.querySelector('.message:last-child');
        m
          ? (setTimeout(() => m.scrollIntoView({ behavior: 'smooth' }), this.transitionDuration), (this.unreadErrors = !1))
          : e === p.ERROR && (this.unreadErrors = !0);
      }),
      g
    );
  }
  updateLog(e) {
    ...
    (t || (t = this.log(p.INFORMATION, '<Log message to update was not found>', !1)),
      Object.assign(t, e),
      S(t.expandedDetails) && (t.expandedDetails = c(t.expandedDetails)),
      this.requestUpdate());
  }
  updated() {
  ...
  canBeExpanded(e) {
    ...
    const t = this.querySelector(`[data\\-id="${e.id}"]`)?.querySelector('.firstrowmessage');
...
d.MAX_LOG_ROWS = 1e3;
u([w()], d.prototype, 'unreadErrors', 2);
u([w()], d.prototype, 'messages', 2);
d = u([y('copilot-log-panel')], d);
...
x = u([y('copilot-log-panel-actions')], x);
const $ = {
    header: 'Log',
...
export { x as Actions, d as CopilotLogPanel };
