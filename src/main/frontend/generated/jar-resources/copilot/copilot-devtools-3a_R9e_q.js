...
var E = Object.defineProperty,
  ...
  b = (e, n, a, s) => {
    for (var o = s > 1 ? void 0 : s ? _(n, a) : n, p = e.length - 1, u; p >= 0; p--) (u = e[p]) && (o = (s ? u(n, a, o) : u(o)) || o);
    return (s && o && E(n, a, o), o);
  };
const S =
    'bg-[linear-gradient(to_right,var(--amber-3),var(--amber-5),var(--amber-3),var(--amber-6))] dark:bg-[linear-gradient(to_right,var(--amber-5),var(--amber-7),var(--amber-5),var(--amber-8))]',
  P =
    'bg-[linear-gradient(to_right,var(--blue-3),var(--blue-5),var(--blue-3),var(--blue-6))] dark:bg-[linear-gradient(to_right,var(--blue-4),var(--blue-6),var(--blue-4),var(--blue-7))]',
  A =
    'bg-[linear-gradient(to_right,var(--ruby-3),var(--ruby-5),var(--ruby-3),var(--ruby-6))] dark:bg-[linear-gradient(to_right,var(--ruby-4),var(--ruby-6),var(--ruby-4),var(--ruby-7))]',
  L =
    'bg-[linear-gradient(to_right,var(--teal-3),var(--teal-5),var(--teal-3),var(--teal-6))] dark:bg-[linear-gradient(to_right,var(--teal-4),var(--teal-6),var(--teal-4),var(--teal-7))]';
let v = class extends x {
  ...
  renderUserButton() {
    const e = i.userInfo?.validLicense,
      n = e ? S : P,
      a = e ? 'text-amber-12 dark:text-amber-11' : 'text-blue-12 dark:text-blue-11',
      s = this.getUserName() !== 'Log in';
    return r`
      <vaadin-button
  ...
  renderWelcomeToVersion() {
    ...
    return e === null
      ? l
      : g.getMostRecentReleaseNoteDismissed()
        ? l
        : e.mostRecentVersion
          ? e.url
            ? r`
  ...
  renderUserImage() {
    return i.userInfo?.portraitUrl
      ? r`<img
        alt="${this.getUserName()}"
        class="rounded-full size-8 object-cover"
        slot="prefix"
        src="https://vaadin.com${i.userInfo.portraitUrl}" />`
      : l;
  }
  renderDevelopmentWorkflow() {
    const e = k(),
      n = C(),
      a = this.getDevelopmentWorkflowConfig(e, n),
      s = a?.bgClass ?? '',
      o = a?.colorClass ?? '',
      p = this.resolveIcon(a),
      u = a?.rotateIcon ? `rotate-180 ${o}` : o,
      h = this.resolveTitle(a),
      f = a?.displayMessage ?? '';
    return r`
      <vaadin-button
...
b([w()], v.prototype, '_helpExpanded', 2);
v = b([m('copilot-devtools')], v);
export { v as CopilotDevTools };
