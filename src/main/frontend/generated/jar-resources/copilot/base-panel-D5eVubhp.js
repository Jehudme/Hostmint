...
class V extends z {
  ...
  repositionInPopover(t) {
    ...
    const n = o.getBoundingClientRect(),
      h = 16,
      d = n.top - h,
      l = window.innerHeight - h - n.bottom,
      m = (t.position ?? t.getAttribute('position') ?? 'bottom').startsWith('top'),
      p = m ? d : l,
      i = m ? l : d;
  ...
  requestLayoutUpdate() {
    ...
    if (M.positionUpdatedManually(t)) return Promise.resolve();
    ...
    return e
      ? new Promise(r => {
          requestAnimationFrame(() => {
            ...
            const h = o._overlayElement,
              d = h?.shadowRoot?.querySelector('[part="overlay"]'),
              l = h?.shadowRoot?.querySelector('[part="content"]'),
              b = h?.shadowRoot?.querySelector('[part="footer"]');
            if (!d || !l) {
              r();
              return;
            }
            const m = Math.max(this.scrollWidth, this.offsetWidth, n.width),
              p = Math.max(this.scrollHeight, this.offsetHeight, n.height),
              i = d.getBoundingClientRect(),
              v = l.getBoundingClientRect(),
              W = Math.max(0, i.width - v.width),
              N = Math.max(0, i.height - v.height),
              s = b?.getBoundingClientRect(),
              P = !!s && s.width > 0 && s.height > 0,
              L = P ? Math.max(0, i.left - s.left) + Math.max(0, s.right - i.right) : 0,
              q = P ? Math.max(0, i.top - s.top) + Math.max(0, s.bottom - i.bottom) : 0,
              a = 16,
              k = this.getPreferredMaxWidth(),
              G = Math.floor((window.innerHeight * 2) / 3),
              F = Math.max(0, this.getPreferredWidth()),
              I = Math.max(0, this.getPreferredHeight()),
              T = Math.min(k, Math.max(120, window.innerWidth - a * 2)),
              $ = Math.min(G, Math.max(120, window.innerHeight - a * 2)),
              f = Math.max(120, Math.min(T, Math.max(F, Math.ceil(m + W + L)))),
              u = Math.max(120, Math.min($, Math.max(I, Math.ceil(p + N + q)))),
              C = `${f}px`,
              E = `${u}px`;
            (o.setAttribute('width', C), o.setAttribute('height', E), (o.width = C), (o.height = E));
            const y = e.position,
              H = Number.parseFloat(o.getAttribute('top') ?? ''),
              R = Number.parseFloat(o.getAttribute('left') ?? ''),
              U = y?.top ?? (Number.isNaN(H) ? 0 : H),
              _ = y?.left ?? (Number.isNaN(R) ? 0 : R),
              B = document.querySelector('copilot-main')?.shadowRoot?.querySelector(`copilot-toolbar #${t}-toolbar-btn`);
            let g = U,
              w = _;
            if (B) {
              const c = B.getBoundingClientRect(),
                S = c.top - 16 - a,
                D = window.innerHeight - a - c.bottom - 16;
              (S >= u || S >= D ? (g = c.top - u - 16) : (g = c.bottom + 16), (w = c.left + c.width / 2 - f / 2));
            }
            ((g = Math.max(a, Math.min(g, window.innerHeight - a - u))),
              (w = Math.max(a, Math.min(w, window.innerWidth - a - f))),
              M.updatePanel(
                t,
                {
                  position: {
