...
function l(o, t) {
  B(o)
    ? (D('show-in-ide', { attach: t ?? !1, goToCustomComponentFile: !0 }),
      c(`${u}show-in-ide`, {
        javaClassName: o.className,
        fileName: o.absoluteFilePath,
      }))
    : P(o)
      ? (D('show-in-ide', { attach: t ?? !1 }), c(`${u}show-in-ide`, { ...S(o), attach: t ?? !1 }))
      : (F('show-in-ide'), c(`${u}show-in-ide`, o));
}
d.on('show-in-ide', o => {
...
function I(o) {
  if (!o.isReactComponent) return;
  const t = w(o.node);
  if (t) return t;
  const e = y(o.node);
  if (e) return e;
  const n = o.children.sort((i, r) => i.siblingIndex - r.siblingIndex).find(i => i.isReactComponent && I(i) !== void 0);
  if (!n) throw new Error(`Could not find the source of ${o.nameAndIdentifier}`);
  return w(n.node);
}
...
function x(o) {
  (p(
    ...
    k('copilot-init-app', { framework: o }, async t => {
      if (t.data.success) (sessionStorage.setItem(f, 'true'), T(), N());
      else {
...
d.on('navigate', o => {
  const t = window.history.state?.idx,
    e = {};
...
class s {
  ...
  getIconTag(t) {
    ...
    if (n === 'IN_PROJECT') return 'thermostatCarbon';
    if (n === 'EXTERNAL') return 'deployedCube';
  }
  getCustomComponentInfo(t) {
    if (t.customComponentData && s.isCustomComponentInstanceInfo(t.customComponentData)) return t.customComponentData;
  }
  isCustomComponent(t) {
  ...
  isVisibleAndSelectable(t) {
    ...
    if (!t.customComponentData) return t.isReactComponent && !t.parent && t.name === 'App' && !e;
    if (e?.uuid === t.uuid) return !1;
    const n = this.getActiveDrillDownData(),
      i = t.customComponentData;
  ...
  getActiveDrillDownContext() {
    if (this.drillDownComponentStack.length !== 0) return this.drillDownComponentStack[this.drillDownComponentStack.length - 1];
  }
  ...
  getActiveDrillDownData() {
    ...
    if (!e?.javaClassName && !e?.reactMethodName) return;
    const n = t.node;
    return {
  ...
  checkNodeIsInDrillDownContext(t, e) {
    return t.createLocationMethodName && e.methodName
      ? t.createLocationMethodName === e.methodName && e.filePath === t.createLocationPath
      : e.filePath === t.createLocationPath && e.className === t.createdClassName;
  }
  persistIntoStorage() {
  ...
  areInternalsVisible(t) {
    if (!this.getCustomComponentInfo(t)) return !0;
    const n = this.getActiveDrillDownData();
    let i;
  ...
  checkChildrenCreateLocationToDisplayInternals(t, e) {
    for (const n of t) {
      ...
      if ((i && i.createLocationPath === e) || this.checkChildrenCreateLocationToDisplayInternals(n.children, e)) return !0;
    }
    return !1;
  }
  ...
  getRouteViewPath() {
    ...
    if (t) return t.customComponentData?.createLocationPath ?? void 0;
  }
