...
export class Flow {
  ...
  async flowInitUi() {
    ...
    return new Promise((resolve, reject) => {
      ...
      const browserDetailsParam = browserDetails ? `&v-browserDetails=${encodeURIComponent(JSON.stringify(browserDetails))}` : '';
      ...
      httpRequest.onerror = () =>
        reject(
          new FlowUiInitializationError(`Invalid server response when initializing Flow UI.
        ${httpRequest.status}
